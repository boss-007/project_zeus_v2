...
% 
function weights=nwbproptrain(train_x,train_t,val_x,val_t,numnodes)
...
p=bbackprop([size(X,2) numnodes size(D,2)],.1,.2,.00002,X,D,VX,VD,2);
weights=p.weights;
end
...
counter=0;
maxiter=10000;
while mse > smse && epochs < maxiter && nwgrad > 1e-9 && counter<4000
    ...
    for i=1:nLayers-1
        ...
        if i < nLayers-1 % inner layers
            ...
            va{i+1} = [2./(1+exp(-vnet{i}(:,1:end-1)))-1 ones(VP,1)];
        else             % output layers
            a{i+1} = 2 ./ (1 + exp(-net{i})) - 1;
            va{i+1} = 2 ./ (1 + exp(-vnet{i})) - 1;
        end
    end
    ...
    if(displaymode==2 && mod(iter,100)==0)
